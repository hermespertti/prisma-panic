...
| # | Scope | Status |
|---|-------|--------|
| 1 | **Playable peeing mechanic** — 3P controller, bladder meter, urgency states (FRESH→SQUEEZY→PRESSING→CRITICAL), coffee/freezer pressure events, toilet relief (vulnerable 3s action), accident → WET PANTS consequence, quota + closing clock, exit endings, HUD, procedural SFX | **BUILDING** |
| 2 | Exploration & movement — real store sections, sprint/crouch movement feel, camera tension feedback, hazards, hiding spots (cereal aisle, fitting room) | next |
| 3 | Roguelike systems — procedural store assembly, random events, upgrades at restocks, risk/reward items, difficulty scaling, multiple endings, run summary | next |
| 4 | Comedy & personality — NPC reactions (shoppers, coffee lady, manager), The Strut + mirrors, wardrobe with mechanical profiles, physical comedy (squeeze-hop, splash particles), environmental storytelling, rare absurd events (the Prisma incident re-enacts in stockroom) | next |
| 5 | Polish — Blender hero model (cap/jeans/belt per reference), lighting pass, sound design pass, juice (screen shake, hit-stop on splash), run summary screen, tutorial moments | next |
