...
| 9 | **Integration soak test** — an AI bot plays complete shifts in the real game (walks/sprints through real doors, real toilets, real perk clicks, crouch-walks near guards, outruns real chases) across seeds + determinism checks. Caught 2 real bugs the unit probes missed: quad pickup re-fired every frame (quota 474, worst when paused on a quad with the picker open) and "QUOTA 4/3" HUD (4-5 items seeded vs quota 3) | **DONE** (M8) |
| 10 | **Feel pass** — the shift has a heartbeat: distance-based footstep cadence for the hero (sprint > walk > crouch, each gait has its own stride length) AND the guards (heel strikes with proximity falloff — a chaser is a different sound than a patrol, and a guard beyond 16u is silent); the near-miss reaction (a chaser closes to a hair's width without landing → whoop + camera flinch, 2.6s cooldown so it doesn't spam per frame); and a click-to-start title card that is a pure cosmetic overlay (the run is live behind it, so no probe or bot path is blocked) whose clock-in is the real audio-unlock gesture (pointer-lock + AudioContext resume) | **DONE** (M9) |
| 11 | **Content deepening** — the store is a bladder map: wet-state ESCALATION (4 tiers at 30/60/90s — the smell radius grows 7→15.4u, tier 2+ staff SMELL you onto alert without a visual, tier 3 is a tannoy CODE PEEP that puts the whole floor hunting, and the fill curve climbs +15/+30/+60% with the tier); a weighted event pool replaces the if/else (CODE PEEP +0.6 fill / ICE BURST fresh slippery patch / SAMPLE CART +10 instant, alongside flicker/puddle/legend); 5 new perks deepening the pool to 10 (ICE WATER fill −10%, TUCKED IN sprint cost 0.8→0.55, NAPKINS waives the wet speed tax, COLD BREW +30 coffee, DEODORANT smell radius ×0.6); and two new store sections — the PRODUCE mist (cold, FILLs you, drifting particles) and the DELI counter (warm, RELAXES) | **DONE** (M10) |
| 12 | **Mobile controls** — the panic fits in a pocket: an ANALOG thumbstick (half tilt = half speed — the push distance is preserved through the movement math, not normalized away like a D-pad), a right-field look zone (drag pans/pitches exactly like the mouse), and SPRINT/CROUCH/USE/BAG buttons that inject the SAME key codes the keyboard uses, so every existing mod, edge-trigger and bladder penalty works untouched. Coarse-pointer devices only — desktop stays clean. Multi-touch proven: three fingers (stick + look + sprint) hold down the loop at once | **DONE** (M11) |
| 13 | **Mobile QA + PWA** — the phone is a real arcade, not an afterthought: installable manifest (fullscreen, theme-colored, maskable icons), offline service-worker shell (network-first navigations so deploys land immediately, cache-first assets so the store plays on a subway), a persisted mute toggle (a phone in public gets silenced and STAYS silenced across reloads), real-viewport layout QA at 390×844 portrait + 844×390 landscape (bladder bar lifted above the thumbstick so the thumb never covers the one number that matters, touch tutorial rewritten — a phone never reads "WASD to move" and believes it), notch safe-areas respected, visual audit by vision model with two overlap bugs caught and fixed. Probes: pp-m12 36/36 | **DONE** (M12) |
| 14 | **Touch soak** — an AI bot plays complete shifts on a fake phone with real CDP fingers: the analog thumbstick, look-zone drags and SPRINT/CROUCH/USE buttons drive the whole loop (keys/yaw/walk hooks wiretapped to zero — proof no keyboard cheat leaks in), three seeds, endings/reports/determinism asserted. It shook out FOUR real bugs: pointer-lock starving touch listeners (game fix: lock only for fine pointers), a bot exit route aiming through a wall (0.07u narrower than the hero), a latch bug beelining it into the break-room, and the deck-ramp camp — engine truth: deck guards FREEZE off-floor (staffStep early-return) and the ramp drops you 1.2u from the anchor, so the escape is a dry walk out the door, a full-sprint straight down the open deck aisle, and relief at the far toilet before the car. The game is beatable on thumbs: seed 1337 CLEAN EXIT rank S in 21s | **DONE** (M13) |
